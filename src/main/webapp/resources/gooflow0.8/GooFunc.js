...
function changeName(obj) {
	$("#workspacename")
			.replaceWith(
					"<input id='inputtitle' type='text' value='"
							+ obj.title
							+ "'   style='border:0px;width:170px;'  class='GooFlow_head_btn' onblur='javascript:replaceLabel(this)'/>");
	moveEnd(document.getElementById("inputtitle"));
}

function replaceLabel(obj) {
	demo.setTitle(obj.value);
	$("#inputtitle").replaceWith(
			"<label id='workspacename' title='newFlow_1' onclick='javascript:changeName(this)'>"
					+ obj.value + "</label>");
	demo.setTitle(obj.value);
...
function saveProperty(id) {
	...
	$("#ele_id").val(id);
	if (model == "line") {
		obj = demo.$lineData[id];
		setValue4Property(obj,source_data.line);
		$("#ele_type").val(obj.M);
		$("#ele_model").val("line");
	} 
	else if (model == "node") {
		obj = demo.$nodeData[id];
        for(var i in source_data)
        {
        	if("line"!=i){
        		 if("base"==i){
                 	setValue4Property(obj,source_data[i]);
                 } 
        		 else {
...
function supplementJson(targeobj,tempobj){
	
	for(var i in tempobj)
	{
		if(i.indexOf("property")>-1){
			var tempchild = tempobj[i];
			for(var j in tempchild)
			{
		 	 	 if(!targeobj.hasOwnProperty(j)){
		 	    	targeobj[j]="";
		 	    }
			}
		}
...
function setValue4Property(obj,sourcedatejson){
		...
		if(i.indexOf("property")>-1){
			...
			for(var j in tempchild)
			{
				 if(obj.hasOwnProperty(j)){
					 	if(tempchild[j].texttype=="number"){
					 		obj[j]=parseInt($("#ele_"+j).val()); 
					 	}else{
			 	    	obj[j]=$("#ele_"+j).val(); 
					 	}
			 	    }
...
function loadSourceData(data) {
	remark = '{';
	for ( var i in data.source_tool) {
		var tempjson = data.source_tool[i];
		for ( var j in tempjson) {
			remark = remark+j+':"'+tempjson[j]+'",';
		}
	}
	var reg=/,$/gi;
