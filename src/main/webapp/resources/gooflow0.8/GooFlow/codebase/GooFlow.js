...
//初始化区域图的对象
	...
	this.$bgDiv.css("ico_chat","{background:url(img/gooflow_icon2.png) no-repeat}");
	...
	if(property.haveHead){
		...
		if(property.headLabel){
	      tmp+="<label id='workspacename' title='"+(property.initLabelText||"工作区名称")+"' onclick='javascript:changeName(this)'"
       +(GooFlow.prototype.color.main? "style='background:"+GooFlow.prototype.color.main+"'" : "")+">"+(property.initLabelText||"工作区名称")+"</label>";
		}
		...
		this.$head.on("click",{inthis:this},function(e){
			...
			switch($(tar).attr("class")){
				case "ico_new":		This.onBtnNewClick();break;
				case "ico_open":	This.onBtnOpenClick();break;
				case "ico_save":	This.onBtnSaveClick();break;
				case "ico_undo":	This.undo();break;
				case "ico_redo":	This.redo();break;
				case "ico_reload":	This.onFreshClick();break;
			}
		});
	}
     ...
     +"<a href='javascript:void(0)' type='direct' class='GooFlow_tool_btn' id='"+this.$id+"_btn_direct'><i class='ico_direct'/></a>"
		...
		if(property.toolBtns&&property.toolBtns.length>0){
			...
			for(var i=0;i<property.toolBtns.length;++i){
				var btn = property.toolBtns[i].split(" ")[0];
				if(source_data.hasOwnProperty(btn) &&source_data[btn].picname != ""){
					tmp+="<a href='javascript:void(0)' type='"+property.toolBtns[i]+"' id='"+this.$id+"_btn_"+btn+"' class='GooFlow_tool_btn'><i style='background:url("+projectname+"/resources/gooflow0.8/GooFlow/codebase/img/"+source_data[btn].picname+"') no-repeat'/></a>";//加入自定义按钮
				}else{
					tmp+="<a href='javascript:void(0)' type='"+property.toolBtns[i]+"' id='"+this.$id+"_btn_"+btn+"' class='GooFlow_tool_btn'><i class='ico_"+property.toolBtns[i]+"'/></a>";//加入自定义按钮
				}
			}
	...
	if(this.$editable){
	  this.$workArea.on("click",{inthis:this},function(e){
		...
		var tempobj={name:"node_"+This.$max,left:X,top:Y,type:This.$nowType};
        
        for(var i in source_data)
        {
        	if("line"!=i){
        		if("base"==i){
                	supplementJson(tempobj,source_data[i]);
                } else {
                	if (This.$nowType == i||This.$nowType.indexOf(i)>-1 ) {
                		supplementJson(tempobj,source_data[i]);
                    } 
                }
        	}
        ...
        This.addNode(This.$id+"_node_"+This.$max,tempobj);
	  ...
	  //格式function(id，type，mark) ,type是单元类型（"node"结点,"line"转换线），mark为布尔值,表示是要标注TRUE还是取消标注FALSE
	  ...
	  if(property.useOperStack&&this.$editable){//如果要使用堆栈记录操作并提供“撤销/重做”的功能,只在编辑状态下有效
		...
		this.onBtnSaveClick=function(){
			var jsonStr2JavaServer = JSON.stringify(demo.exportData());
			
			$.ajax({
				  type: "Post",     
				  url: '/demo/queryById4',
				  data:{savefilestr:jsonStr2JavaServer},
		...
		this.onBtnOpenClick=function(){
			$.ajax({
				  type: "POST",     
				  url: '/demo/queryById3',
				  data:{tempId:"3",tempType:"2"},
				  success:function(data) {  
					  window.location.href=projectname+"/queryById?tempId=3&tempType=2";
				  },   
				  error: function (jqXHR, textStatus, errorThrown) {
			            /*错误信息处理*/
					  alert("失败");
        ...
        this.onFreshClick = function () {
            demo.clearData();
			demo.loadData(JSON.parse(jsondatastrbyserver));
			this.$undoStack.length=0;
        };
...
//		json.to="";
		...
		if(json.type.indexOf(" round")<0){
			...
			if(source_data.hasOwnProperty(json.type) &&source_data[json.type].picname != ""){
				this.$nodeDom[id]=$("<div class='GooFlow_item"+mark+"' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='1' style='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'><tr><td class='ico'>" +
						"<i style='background:url("+projectname+"/resources/gooflow0.8/GooFlow/codebase/img/"+source_data[json.type].picname+"') no-repeat'/></td><td>"+json.name+"</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
			}else{
			...
			var type =json.type.split(" ")[0]
			if(source_data.hasOwnProperty(type) &&source_data[type].picname != ""){
				json.width=24;json.height=24;
				this.$nodeDom[id]=$("<div class='GooFlow_item item_round"+mark+"' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='0'><tr><td class='ico'><i style='background:url("+projectname+"/resources/gooflow0.8/GooFlow/codebase/img/"+source_data[type].picname+"') no-repeat'/></td></tr></table><div  style='display:none'><div class='rs_close'></div></div><div class='span'>"+json.name+"</div></div>");
			}else{
				json.width=24;json.height=24;
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".GooFlow_item","mouseup",{inthis:this},function(e){
			...
			if(lineStart&&!This.$mpTo.data("p")){
				var tempobj={from:lineStart.id,to:this.id,name:""};
		        supplementJson(tempobj,source_data.line);
		        This.addLine(This.$id+"_line_"+This.$max,tempobj);
				This.$max++;
			}
			else{
				if(lineStart){
